	...
	class PR_DHL_WC_Order_eCS_Asia extends PR_DHL_WC_Order
	{
		...
		public function additional_meta_box_fields($order_id, $is_disabled, $dhl_label_items, $dhl_obj)
		{
			...
			woocommerce_wp_text_input(array(
				...
				'label'       		=> __('Insurance Value:', 'dhl-for-woocommerce'),
				'placeholder' 		=> '',
				'description'		=> '',
			...
			if ($this->is_shipping_domestic($order_id))
			{
				...
				woocommerce_wp_checkbox(array(
					...
					'label'       		=> __('Open Box Service:', 'dhl-for-woocommerce'),
					'placeholder' 		=> '',
					'description'		=> '',
		...
		protected function get_label_args_settings($order_id, $dhl_label_items)
		{
			...
			if (!empty($dhl_label_items['pr_dhl_tax_id_type']))
			{
				...
				if ($dhl_label_items['pr_dhl_tax_id_type'] == '3')
				{ //If IOSS
					if (!$this->is_crossborder_shipment_europe($order_id))
					{
				...
				elseif ($dhl_label_items['pr_dhl_tax_id_type'] == '1')
				{ // If VAT/GST
					if (!$this->is_crossborder_shipment_europe($order_id) && !$this->is_crossborder_shipment_great_britain($order_id))
					{
						$args['dhl_settings']['dh_tax_id'] = '';
				...
				elseif ($dhl_label_items['pr_dhl_tax_id_type'] == '2')
				{ // IF EORI
					if (!$this->is_crossborder_shipment_great_britain($order_id))
					{
						$args['dhl_settings']['dh_tax_id'] = '';
	...
	*/
		...
		public function get_bulk_actions()
		{
			...
			$shop_manager_actions = array(
				...
				// 'pr_dhl_closeout_all'      => __( 'DHL Close Out All', 'dhl-for-woocommerce' ),
				'pr_dhl_closeout_selected'      => __('DHL Close Out Selected', 'dhl-for-woocommerce')
			);
				...
				try
				{
					...
					if (isset($closeout['file_info']->url))
					{
						...
						$manifest_text 	= sprintf(
							...
							$label_url,
							__('Download Closeout File', 'dhl-for-woocommerce') . ' ' . $closeout['handover_id']
						);
					}
					else
					{
						$manifest_text = __('Handover ID : ', 'dhl-for-woocommerce') . $closeout['handover_id'];
					}

					$message = sprintf(
						__('Finalized DHL Close Out - %2$s', 'dhl-for-woocommerce'),
